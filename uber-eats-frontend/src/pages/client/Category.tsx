...
import {
  category,
  categoryVariables,
} from "../../__api_schema_typed__/category";
...
const Category = () => {
  ...
  const [loadCategory, { loading, error, data }] = useLazyQuery<
    category,
    categoryVariables
  >(CATEGORY_QUERY);
  useEffect(() => {
    ...
    <div className="bg-gray-100 h-full">
      <Container>
        {!loading && data?.category?.category && (
          <div className="flex flex-col ml-10 pb-96">
            <div className="flex mx-auto mt-10 items-center">
              <img
                src={data?.category?.category?.coverImage}
                alt="category"
                className="w-20 h-20 rounded-full mr-10 shadow-lg"
              />
