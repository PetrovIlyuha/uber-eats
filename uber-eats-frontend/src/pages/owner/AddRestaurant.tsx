...
} from "../../__api_schema_typed__/createRestaurant";
...
const AddRestaurant = () => {
  ...
  const [categoriesAutoCompletions, setCategoriesAutoCompletions] = useState<
    AutoCompleteOption[]
  >([]);

  const [create, { data }] = useMutation<
    createRestaurant,
    createRestaurantVariables
  ...
  const { data: categories } = useQuery<allCategories>(GET_ALL_CATEGORIES);
  const {
    register,
    getValues,
    setValue,
    ...
    setError,
    handleSubmit,
  } = useForm<CreateRestaurantFormInterface>({ mode: "onBlur" });
  ...
  const createOnSubmit = async () => {
    ...
    let categoryImageWhiteSpaceConstraint =
      categoryImage?.[0]?.name.includes(" ") || false;
    ...
    if (categoryImageWhiteSpaceConstraint) {
      setError("categoryImage", {
        type: "manual",
        message: "Category image file can't contain whitespaces",
      });
      return;
    }
    ...
    const { imageUrl: coverImageUrl } = await (
      await fetch("http://localhost:4000/uploads", {
        method: "POST",
        body: formBody,
      })
  ...
  useEffect(() => {
    if (
      ...
      setCategoriesAutoCompletions(
        categories?.allCategories?.categories?.map((c) => ({
          name: c.name,
          image: c.coverImage,
          key: c.name,
        }))
      ...
      <Container>
        ...
        >
          ...
          <form
            ...
            className="w-6/12 flex flex-col h-3/6 justify-between mb-10 shadow-lg p-10 bg-white rounded-lg"
          >
            <input
              name="name"
            ...
            <input
              name="address"
              ref={register({ required: "Address should be specified" })}
              className="input w-full mt-10 p-3"
              type="text"
              placeholder="Restaurant Legal Address"
            />
            <div className="mt-2">
              {errors.address && (
                <FormError message={`Address should be specified`} />
              )}
            </div>
            <label className="text-emerald-700 mt-8" htmlFor="image">
              Select cover image for the restaurant
            </label>
            <input
              ...
              name="image"
              accept="image/*"
              className="input w-full mt-2 p-3"
              placeholder="select image for restaurant"
            />
            {errors.image && errors.image.type === "required" && (
              <FormError message="Image should be provided" />
            )}
            ...
            />
            <h2 className="mb-4 text-emerald-700">
              Or create new Category below
            </h2>
            <input
              name="categoryName"
              ref={register({ required: "You should provide category name" })}
              className="input w-full my-5 p-3"
              type="text"
              placeholder="Restaurant category name"
            />
            {errors?.categoryName && (
              <FormError message="Category name should be selected" />
            )}
            ...
            <input
              ...
              type="file"
            />
            {errors.categoryImage?.type === "manual" && (
              <FormError message="Category image file can't contain whitespaces" />
            )}
            <MainButton
