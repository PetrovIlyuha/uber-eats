...
import { createDish, createDishVariables } from '../../__api_schema_typed__/createDish'
import { QUERY_MY_RESTAURANT } from './SingleRestaurant'
import { AiOutlineDelete } from 'react-icons/ai'
import { FaHandPointer } from 'react-icons/fa'
import NoCoverSelected from '../../images/cover_not_select.svg'
import { DishChoiceInputType, DishOptionInputType } from '../../__api_schema_typed__/globalTypes'
...
const CreateDish = () => {
	...
	const { getValues, errors, setError, handleSubmit, register, watch } = useForm<CreateMenuItemInterface>({ mode: "onBlur" })
	...
	function onCompleted(data: createDish) {
		const { createDish: { ok, error } } = data
		if (ok) {
	...
	const addOption = () => {
		setOptions([...options, { id: Math.random().toString(36).slice(0, 6), name: '', extra: 0 }])
	}
	const removeOption = (id: string | undefined) => {
		setOptions(options.filter(op => op.id !== id))
	}
	const setOptionsDetails = (e: React.ChangeEvent<HTMLInputElement>, id: string | undefined) => {
		setOptions(options.map(op => op.id === id ? { ...op, [e.target.name]: e.target.name === 'extra' ? +e.target.value : e.target.value } : op))
	}

	const setVariantsForOption = (id: string | undefined) => {
		setOptions(options.map(op => op.id === id && op.choices ?
			{
				...op, choices:
					[...op.choices,
	...
	const setVariantDetail =
		(e: React.ChangeEvent<HTMLInputElement>,
			...
			setOptions(options.map(op => op.id === optionId ?
				{
					...op, choices: op.choices?.map(c => c.id === variantId ?
						{
							...c,
							[e.target.name]: e.target.name === 'extra' ? +e.target.value : e.target.value
						} : c)
				} : op))
		}

	const removeOptionChoice = (optionId: string | undefined, choiceId: string | undefined) => {
		setOptions(options.map(op => op.id === optionId ?
	...
	return (
		<div className="bg-gray-100 h-full">
			<Container>
				...
				>
					<h2 className="my-10 text-2xl font-extrabold text-gray-600">Create Menu Item</h2>
					<form
						...
						<input
							...
							className="input w-full p-3 mt-8" type="text" placeholder="Menu item name"
						/>
						<div className="mt-3">
							{errors.name && <FormError message="Name is a required field and no shorter than 3 letters" />}
						</div>
						<input
							name="price"
							ref={register({ required: "Price should be specified" })}
							min={1}
							className="input w-full p-3 mt-8" type="number" placeholder="Menu item price"
						/>
						<div className="mt-3">
							{errors.price && <FormError message="Price is a required field" />}
						</div>
						<textarea
							cols={10}
							name="description"
						...
						<div className="mt-4 flex items-center flex-col-2">
							{previewImage ? (
								<img src={`${previewImage}`} className="w-1/2 rounded-md mr-2 h-32 sm:h-48" />
							) : (
								<img src={NoCoverSelected} alt="cover not selected area" className="w-1/2 rounded-md mr-2 h-48" />
							)}
							<div>
								<label
						...
						<div className="flex flex-col">
							{options.length > 0 && options.map(option => (
								<motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} key={option.id}>
									<div className="mt-3 flex justify-between">
										<div className="p-2 flex items-center justify-center align-center bg-emerald-200 rounded-md shadow-md">
											<div className="w-full max-w-xs mx-auto">
												<div>
													<label htmlFor="name" className="text-sm font-medium text-gray-700">Option</label>
													<div className="mt-1 rounded-md shadow-sm">
														<input
															type="text"
															name="name"
															onChange={(e) => setOptionsDetails(e, option?.id)}
										...
										<div className="p-2 flex items-center justify-center bg-emerald-200 rounded-md shadow-md">
											<div className="w-full max-w-xs mx-auto">
												<div>
													<label htmlFor="company_website" className="text-sm font-medium text-gray-700">Extra price</label>
													<div className="mt-1 rounded-md shadow-sm">
														<input
															type="number"
															name="extra"
									...
									<div className="flex items-center">
										...
										>
											<span
												className="animate-pulse text-white font-semibold">
												Create Option Variants
											</span>
										</button>
										<h2 className="font-semibold mt-4 justify-self-end ml-10">{option?.name.length > 0 &&
											<span className="font-bold flex text-emerald-700">
												<FaHandPointer size={20} className="mr-2 transform -rotate-45" />
												for "{option?.name}"</span>}
										</h2>
									</div>
									{option?.choices?.length !== 0 && option?.choices?.map(choice => {
										return <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="mt-2 ml-2 flex justify-between">
											<div className="p-2 flex items-center justify-center align-center bg-gray-200 rounded-md shadow-md">
												<div className="w-full max-w-xs mx-auto">
													<div>
														<label htmlFor="company_website" className="text-sm font-medium text-gray-700">Variant name</label>
														<div className="mt-1 rounded-md shadow-sm">
															<input
																type="text"
																name="name"
																onChange={(e) => setVariantDetail(e, option?.id, choice?.id)}
																className="focus:ring-indigo-500 focus:border-indigo-500 w-full  py-1 sm:pl-2 sm:text-sm border-gray-500 rounded-md" placeholder="www.example.com"
															/>
														</div>
													</div>
												</div>
													...
													<div>
														...
														<div className="mt-1 rounded-md shadow-sm">
															<input
																...
																onChange={(e) => setVariantDetail(e, option?.id, choice?.id)}
																min={0}
																className="focus:ring-indigo-500 focus:border-indigo-500 block w-full h-full py-1 sm:pl-2 sm:text-sm border-gray-500 rounded-md" placeholder="Extra price"
															/>
														</div>
													</div>
												</div>
